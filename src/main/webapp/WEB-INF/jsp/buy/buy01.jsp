	...
	<%@ page import="java.util.Vector" %>
		<%@ page import="beans.BookBean" %>
			<%@ page import="beans.MemberBean" %>
				...
				<% request.setCharacterEncoding("UTF-8"); //id 기본으로 0 설정 int userid=0; String account=null; String name=null;
					String phone=null; String zipcode=null; String address=null; String detailAddress=null; int point=0; String
					imgUrl=null; String tit=null; int price=0; int bookid=0; int orderNum=0; String[] bookids=null; String[]
					orderNums=null; if(request.getParameterValues("bookids")==null &&
					request.getParameterValues("orderNums")==null){ //배열 데이터가 없는 경우 단일데이터(바로 구매 버튼 통해서 들어오는 경우)
					bookid=Integer.parseInt(request.getParameter("bookid"));
					orderNum=Integer.parseInt(request.getParameter("orderNum")); }else{ //배열 데이터가 있는 경우 (장바구니를 통해서 들어오는 경우)
					bookids=request.getParameterValues("bookids"); orderNums=request.getParameterValues("orderNums"); } //세션에서 아이디
					값 int로 가져오기 if(session.getAttribute("idKeyS") !=null){ //세션값 int로 저장 Object
					sessionValue=session.getAttribute("idKeyS"); // 타입 확인 후 변환 if (sessionValue instanceof String) { String
					strValue=(String) sessionValue; // String으로 캐스팅 try { userid=Integer.parseInt(strValue); } catch
					(NumberFormatException e) { e.printStackTrace(); // 변환 실패 시 예외 처리 } } else if (sessionValue instanceof
					Integer) { // 만약 세션에 직접 Integer로 저장되어 있다면 userid=(Integer) sessionValue; } } //회원 정보 가져오기 Vector<MemberBean>
					mlist = null;
								...
								<form name="buyFrm" method="post"> <!-- 결제수단 쿼리스트링으로 JS에서 action 설정 -->
									<ul class="payment-info">
										<li>
											...
											<ol>
												<li>
													<input type="text" name="buyName" id="buyName" placeholder="주문자의 이름을 입력해 주세요."
														value="<%=name%>" required />
												</li>
												<li>
													<input type="text" name="buyPhone" id="buyPhone" placeholder="주문자의 휴대폰 번호를 '-' 없이 입력해 주세요."
										...
										<li>
											...
											<ol>
												<li>
													<input type="text" name="receiveName" id="receiveName" placeholder="받는 분의 이름을 입력해 주세요."
														required />
												</li>
												...
												<li>
													<input type="text" name="detailAddress" id="detailAddress" placeholder="상세 주소를 입력해 주세요."
														required />
												</li>
											</ol>
										</li>
										<li>
											...
											<ol class="buy-products">
												<% if(bookids==null && orderNums==null){//단일데이터 //책 정보 가져오기 Vector<BookBean> vlist = null;
													...
													<% }else{//배열데이터 for(int i=0; i<bookids.length; i++){ //책 정보 가져오기 Vector<BookBean> vlist =
														null;
